...
func (uq *UserQuery) QueryManager() *UserQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withManager:      uq.withManager.Clone(),
...
func (uq *UserQuery) WithManager(opts ...func(*UserQuery)) *UserQuery {
	query := (&UserClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withManager = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withManager; query != nil {
		if err := uq.loadManager(ctx, query, nodes, nil,
			func(n *User, e *User) { n.Edges.Manager = e }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadManager(ctx context.Context, query *UserQuery, nodes []*User, init func(*User), assign func(*User, *User)) error {
...
func (uq *UserQuery) loadSubordinates(ctx context.Context, query *UserQuery, nodes []*User, init func(*User), assign func(*User, *User)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldManagerID)
	}
...
func (uq *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := uq.ctx.Fields; len(fields) > 0 {
		...
		if uq.withManager != nil {
			_spec.Node.AddColumnOnce(user.FieldManagerID)
		}
	}
